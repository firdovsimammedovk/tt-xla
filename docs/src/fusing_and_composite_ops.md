...
```{literalinclude} ../../python_package/tt_torch/composite_ops.py
:language: python
:lines: 30-47
```
...
```{literalinclude} ../../python_package/tt_torch/backend/passes.py
:language: python
:lines: 135-180
```
...
```{literalinclude} ../../python_package/tt_torch/fusion_providers.py
:language: python
:lines: 20-80
```
...
Subclasses may override `match_filter` to inspect each match and reject it based on hardware constraints (for example using `node.meta["example_value"]` for tensor shapes at match time). The default implementation accepts all matches:
